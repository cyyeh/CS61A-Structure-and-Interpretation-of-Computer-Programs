...
def replace_elem(lst, index, elem):
    ...
    return [element if i != index else elem for element, i in zip(lst, range(len(lst)))]
...
def put_piece(board, max_rows, column, player):
    ...
    for row_index in range(max_rows - 1, -1, -1):
        if get_piece(board, row_index, column) == '-':
            return (row_index, [row if i != row_index else replace_elem(row, column, player) for row, i in zip(board, range(len(board)))])
        elif get_piece(board, row_index, column) != '-' and row_index == 0:
            return (-1, board)    
