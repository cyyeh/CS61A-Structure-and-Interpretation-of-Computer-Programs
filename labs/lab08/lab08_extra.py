...
def trade(first, second):
    ...
    while m < len(first) and n < len(second):
        if sum(second[:n]) == sum(first[:m]): # change this line!
            first[:m], second[:n] = second[:n], first[:m]
            return 'Deal!'
        elif sum(second[:n]) > sum(first[:m]):
            m += 1
        elif sum(second[:n]) < sum(first[:m]):
            n += 1
        else:
...
def make_to_string(front, mid, back, empty_repr):
    ...
    '()'
    """
    def create_representation(lst):
        if lst is Link.empty:
            return empty_repr
        else:
            return front + str(lst.first) + mid + create_representation(lst.rest) + back
...
def tree_map(fn, t):
    ...
    if t.is_leaf():
        ...
        for branch in t.branches:
            new_branches.append(tree_map(fn, branch))
...
def long_paths(tree, n):
    ...
    def create_long_path(tree, path, path_length):
        if tree.is_leaf():
            if path_length >= n:
                paths.append(path)
        else:
            ...
            for branch in tree.branches:
                new_path = insert(path, branch.label, path_length)
                create_long_path(branch, new_path, path_length + 1)

    create_long_path(tree, -1, 0)
