...
def is_weight(w):
    ...
    "*** YOUR CODE HERE ***"
    return is_leaf(w) and not is_mobile(w)
...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    return total_weight(end(sides(m))) * 2 == total_weight(m)
...
class Account:
    ...
    def time_to_retire(self, amount):
        ...
        import math
        result = 1
        base = amount / self.balance
        while math.pow(1 + self.interest, result) < base:
            result += 1
...
def make_withdraw(balance, password):
    ...
    password = password
    attempts = []
    def make_protected_withdraw(money, pwd):
        nonlocal balance
        nonlocal attempts
    ...
    return make_protected_withdraw
